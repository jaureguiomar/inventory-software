...
import { SupplierResponse, WindowResponse, Supplier } from "../interfaces/supplier/supplier";
...
export default defineComponent({
   ...
   setup() {
      ...
      const onRefreshData = () => {
         ...
         axios.get<SupplierResponse>("http://127.0.0.1/inventory-software-api/api/supplier/v3/select-all.php")
            .then((response) => {
               if(response) {
      ...
      const onClienSeeWindowClick = (item:Supplier) => {
         window.api.send("supplier-module-window", {
            id: item.id,
            type: "see",
            data: {
