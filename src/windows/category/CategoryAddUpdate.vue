...
export default defineComponent({
   ...
   setup() {
      ...
      window.api.receive("category-module-window-reply", (data:IPCParamsContent) => {
         ...
         create_activity_log({
            name: `The user has access to category ${(page.id > 0) ? 'add' : 'update'} report`,
            extra_data: JSON.stringify(category),
            id_operation: ACTIVITY_LOG_ACCESS.ACCESS,
            id_access: ACTIVITY_LOG_OPERATION.CATEGORY_REPORT_ADD_UPDATE,
      ...
      const onAddUpdate = async() => {
         ...
         if(page.id <= 0) {
            if(getIsOnline.value) {
               try {
                  let response = await axios.put<CategoryResponse>(`${ getServer.value }/category`,
                     {
            ...
            if(getIsOnline.value) {
               try {
                  let response = await axios.post<CategoryResponse>(`${ getServer.value }/category`,
                     {
                        id: page.id,
                        name: field.name.text,
                        id_user: getSessionUserId.value,
