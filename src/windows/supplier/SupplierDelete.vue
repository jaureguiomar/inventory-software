      ...
      div.main-container(v-if="loaded" key="content")
         ...
         Menu
            template(#left-content)
               p {{ $t("supplier.window.delete.title") }}
            template(#subtitle) {{ $t("supplier.window.delete.subtitle") }}
         Content
            template(#content)
               ...
               div.text-center
                  button( type="submit" class="btn btn-danger text-center mr-2" @click="onDelete") {{ $t("supplier.window.delete.button.delete") }}
